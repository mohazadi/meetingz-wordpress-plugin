...
class Meetingz_Display_Helper {
	...
	public function get_join_form_as_string( $room_id, $meta_nonce, $access_as_moderator, $access_as_viewer, $access_using_code ) {
		...
		ob_start();
		include $this->file . 'partials/meetingz-join-display.php';
	 ...
	 * @param   Boolean $manage_mtz_recordings              User capability to manage recordings.
	 ...
	 */
	public function get_collapsable_recordings_view_as_string( $room_id, $recordings, $manage_mtz_recordings, $view_extended_recording_formats ) {
		$html_recordings = $this->get_recordings_as_string( $room_id, $recordings, $manage_mtz_recordings, $view_extended_recording_formats );
		ob_start();
		include $this->file . 'partials/meetingz-collapsable-recordings-display.php';
	 ...
	 */
	private function get_recordings_as_string( $room_id, $recordings, $manage_mtz_recordings, $view_extended_recording_formats ) {
		$columns = 5;
		if ( $manage_mtz_recordings ) {
			$columns++;
		}
		$sort_fields = $this->set_order_by_field();
		ob_start();
		$meta_nonce                   = wp_create_nonce( 'mtz_manage_recordings_nonce' );
		$date_format                  = ( get_option( 'date_format' ) ? get_option( 'date_format' ) : 'Y-m-d' );
		$default_mtz_recording_format = 'presentation';
		include $this->file . 'partials/meetingz-recordings-display.php';
	...
	private function set_order_by_field() {
		$sort_asc_classes   = 'dashicons dashicons-arrow-up-alt2 mtz-header-icon';
		$sort_desc_classes  = 'dashicons dashicons-arrow-down-alt2 mtz-header-icon';
		$sort_meta_nounce   = wp_create_nonce( 'mtz_sort_recording_columns_nonce' );
		...
		if ( isset( $_GET['order'] ) && isset( $_GET['orderby'] ) && isset( $_GET['nonce'] ) && wp_verify_nonce( $_GET['nonce'], 'mtz_sort_recording_columns_nonce' ) ) {
			$new_direction    = ( sanitize_text_field( $_GET['order'] ) == 'asc' ? 'desc' : 'asc' );
			$new_sort_classes = ( 'asc' == $new_direction ? $sort_desc_classes : $sort_asc_classes ) . ' mtz-current-sort-icon';
			...
			if ( array_key_exists( $selected_field, $custom_sort_fields ) ) {
				$custom_sort_fields[ $selected_field ] = (object) array(
					...
					'header_classes' => 'mtz-column-header-highlight',
				);
			}
		}
		...
		foreach ( $custom_sort_fields as $field => $values ) {
			if ( null === $custom_sort_fields[ $field ] ) {
				$custom_sort_fields[ $field ] = (object) array(
					...
					'classes'        => $sort_asc_classes . ' mtz-hidden',
					'header_classes' => 'mtz-recordings-unselected-sortable-column',
				);
			}
		}
	...
	public function get_room_list_dropdown_as_string( $rooms, $selected_room, $html_form ) {
		ob_start();
		include $this->file . 'partials/meetingz-room-dropdown-display.php';
