 ...
 */
class Meetingz_Recording_Helper {
	...
	public function get_recordings_based_on_capability( $room_ids ) {
		...
		if ( $manage_recordings ) {
			$this->recordings = Meetingz_Api::get_recordings( $room_ids, 'published,unpublished' );
		} else {
			$this->recordings = Meetingz_Api::get_recordings( $room_ids, 'published' );
		}
		return $this->recordings;
	}
	...
	private function filter_managed_recording( $recording ) {
		if ( 'true' == $recording->protected ) {
			$recording->protected_icon_classes = 'mtz-icon mtz_protected_recording is_protected dashicons dashicons-lock';
			$recording->protected_icon_title   = __( 'Protected', 'meetingz' );
		} elseif ( 'false' == $recording->protected ) {
			$recording->protected_icon_classes = 'mtz-icon mtz_protected_recording not_protected dashicons dashicons-unlock';
			$recording->protected_icon_title   = __( 'Unprotected', 'meetingz' );
		}

		if ( 'true' == $recording->published ) {
			$recording->published_icon_classes = 'mtz-icon mtz_published_recording is_published dashicons dashicons-visibility';
			$recording->published_icon_title   = __( 'Published' );
		} else {
			$recording->published_icon_classes = 'mtz-icon mtz_published_recording not_published dashicons dashicons-hidden';
			$recording->published_icon_title   = __( 'Unpublished' );
		}

		$recording->trash_icon_classes = 'mtz-icon mtz_trash_recording dashicons dashicons-trash';
		return $recording;
	}
