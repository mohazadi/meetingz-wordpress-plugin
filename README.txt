=== MeetingZ ===
Contributors: est, cloudest, meetingz, jfederico, yfngdu
Website: https://meetingz.ir
Tags: meetingz, cloudest, blindsidenetworks, bigbluebutton, opensource, open source, web, conferencing, webconferencing
...
This lightweight plugin integrates MeetingZ functionality into Wordpress.

== Description ==

[MeetingZ](https://meetingz.ir/ "MeetingZ") is an open source web conferencing system. This plugin integrates MeetingZ into WordPress allowing bloggers to create and manage meetings rooms to interact with their readers. It was developed and is maintained by <a href="https://cloudest.ir/" target="_blank">Cloudest</a>.

For more information on setting up your own MeetingZ server or for using an external hosting provider visit [https://meetingz.ir/support](https://meetingz.ir/support "https://meetingz.ir/support").
...
Here's an overview of the installation.
   ...
   1. Click Add new.
   1. In the search bar enter "meetingz" and click search plugins.
   1. When you find the plugin called MeetingZ by EST click the install now link.
   ...
   1. Find the Rooms Widget. Then click and drag it to either the right, content, or footer windows on the right of the screen depending on where you wish the MeetingZ widget to appear.
   ...
   1. Fill out the URL of where the MeetingZ server is running (be sure to add / to the end of the URL) and its salt. Then click on Save.
...
After activating the MeetingZ plugin, click on "Rooms" and "Add New" and give the meeting a title and fill out the room details.
...
For setting up in the sidebar, add the meetingz widget, as you do with any other widget, dragging the box to the position you want it to be in.

By default each room will be on its own page, with the permalink listed in the Rooms page on the admin panel. Rooms can also be inserted into any post/page using the [meetingz] shortcode with specified tokens, token1, token2 in the shortcode in the format, [meetingz token="token1,token2"].
...
To place recordings on a separate post/page, use the shortcode [meetingz type="recording"], with the room tokens of the desired recordings. For example, the recordings for the tokens, token1, token2 can be displayed using [meetingz type="recording" token="token1, token2"].
...
To allow another user to create and edit rooms, assign them a role which has the permissions, activate_plugins and edit_mtz_rooms, publish_mtz_rooms, delete_mtz_rooms, delete_published_mtz_rooms, and edit_published_mtz_rooms. The permission structure is similar for posts and pages.
...
To allow another user to join as moderator, viewer, or with a code, assign them to a role with one of the corresponding permissions, join_as_moderator_mtz_room, join_as_viewer_mtz_room, or join_with_password_mtz_room. By default, the owner of the room will always join their rooms as a moderator. The default does not apply to others' rooms.

To allow another user to manage recordings, assign them to a role which has the permissions, manage_mtz_room_recordings.

To allow another user to use shortcodes or the widget, assign them to a role which has the permissions, edit_mtz_rooms.
...
Since version 1.3.4 it is possible to provide direct access to the meeting rooms by adding the meeting token ID to the shortcode: (eg. [meetingz token="aa2817f3a1e1"]).
...
Make sure you are using MeetingZ 0.8 or higher. Ensure the server settings are configured correctly.
...
* Fixed issue. Fixed issue with custom roles not being considered for matching with MeetingZ roles.
...
* Fixed issue. Increased the interval for polling meetings when waiting for moderator is used.
...
* Fixed issue. When using short codes, the meetingz content appeared at the very top on the page or post.
...
* Changed meetingId. Wordpress meetingID is no longer the meetingID. Instead a short 13 characters internal token is used to generate the real meetingID.
* Added feature. Title on recording list can be set up using a shortcode parameter [meetingz_recordings title='Example'].
...
* Added feature. Extended shortcode can receive token id and submit message [meetingz token="a7ccc7f752f65" submit"Meet me there!"]. [token] can be taken from the list of meeting rooms created, when set the join button will link to the specific meeting. [submmit] will override the text in the join button.
...
* Changed permissions. Administrator can now set moderator or attendee as default mtz-roles to the different wp-roles available.
...
* Added a warning to the welcome message on the meetingz chat box when the meeting is recorded.
* A generic welcome message can be set as parameter using the shortcode [meetingz welcome='<br>Custom message<br>%%CONFNAME%%'].

= 1.3.1 =
* Changed version control. 1:major version (remains),2:minor version (former release version),3:release version.
* Added shortcode [meetingz] to render an access form into a page or post.
...
* Admin users can publish/unpublish and delete recordings from the MeetingZ server.
* Added shortcode [meetingz_recordings] to render the list of recordings into a page or post.
