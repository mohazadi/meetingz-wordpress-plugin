...
class Meetingz_Activator {
	...
	public static function set_default_roles() {
		$are_defaults_set = get_option( 'meetingz_default_roles_set' );
		...
		update_option( 'meetingz_default_roles_set', true );
	}
	...
	private static function set_admin_capability( $role ) {
		self::set_edit_room_capability( $role );
		$role->add_cap( 'edit_others_mtz_rooms' );
		$role->add_cap( 'delete_others_mtz_rooms' );
		$role->add_cap( 'create_recordable_mtz_room' );
	...
	private static function set_edit_room_capability( $role ) {
		$role->add_cap( 'edit_mtz_rooms' );
		$role->add_cap( 'edit_published_mtz_rooms' );
		$role->add_cap( 'delete_mtz_rooms' );
		$role->add_cap( 'delete_published_mtz_rooms' );
		$role->add_cap( 'publish_mtz_rooms' );
	...
	private static function join_permissions_set( $role ) {
		if ( $role->has_cap( 'join_as_moderator_mtz_room' ) || $role->has_cap( 'join_as_viewer_mtz_room' ) || $role->has_cap( 'join_with_access_code_mtz_room' ) ) {
			return true;
		}
		return false;
	}
