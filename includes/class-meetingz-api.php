<?php
/**
 * Static API calls to the Meetingz Server.
 *
 ...
 * Static API calls to the Meetingz Server.
 *
 * This class defines all code necessary to interact with the remote Meetingz server.
 *
 * @since      3.0.0
 * @package    Meetingz
 * @subpackage Meetingz/includes
...
class Meetingz_Api {
	...
	public static function create_meeting( $room_id, $logout_url ) {
		...
		if ( get_post( $rid ) === false || 'mtz-room' != get_post_type( $rid ) ) {
			return 404;
		}

		$name           = html_entity_decode( get_the_title( $rid ) );
		$moderator_code = get_post_meta( $rid, 'mtz-room-moderator-code', true );
		$viewer_code    = get_post_meta( $rid, 'mtz-room-viewer-code', true );
		$recordable     = get_post_meta( $rid, 'mtz-room-recordable', true );
		$meeting_id     = get_post_meta( $rid, 'mtz-room-meeting-id', true );
		$arr_params     = array(
			'name'        => esc_attr( $name ),
	...
	public static function get_join_meeting_url( $room_id, $username, $password, $logout_url = null) {
		...
		if ( ! self::is_meeting_running( $rid ) ) {
			...
			if ( 200 !== $code ) {
				wp_die( esc_html__( 'It is currently not possible to create rooms on the server. Please contact support for help.', 'meetingz' ) );
			}
		}
	...
	/**
	 * Verify that the endpoint is a MeetingZ server, the salt is correct, and the server is running.
	 *
	 * @since 3.0.0
	 *
	 * @param String $url         MeetingZ URL endpoint to be tested.
	 * @param String $salt        MeetingZ server salt to be tested.
	 *
	 * @return Boolean true|false Whether the MeetingZ server settings are correctly configured or not.
	 */
	public static function test_meetingz_server( $url, $salt ) {
	...
	/**
	 * Returns the complete url for the meetingz server request.
	 *
	 * @since   3.0.0
	 *
	 * @param   String $request_type   Type of request to the meetingz server.
	...
	private static function build_url( $request_type, $args ) {
		...
		$url_val  = strval( get_option( 'meetingz_url', 'https://api.meetingz.ir/<user>/' ) );
		$salt_val = strval( get_option( 'meetingz_salt', '8cd8ef52e8e101574e400365b55e11a6' ) );
