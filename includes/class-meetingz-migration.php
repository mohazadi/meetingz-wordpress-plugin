...
class Meetingz_Migration {
	...
	private function import_rooms() {
		global $wpdb;
		$old_rooms_table     = 'wp_meetingz';
		$old_room_logs_table = 'wp_meetingz_logs';
		...
		if ( $wpdb->get_var( $old_rooms_query ) === $old_rooms_table ) {
			...
			foreach ( $old_rooms as $old_room ) {
				...
				if ( 0 === $new_room_id ) {
					$this->error_message = sprintf( wp_kses( __( 'Failed to import the room, %s.', 'meetingz' ), array() ), $old_room->meetingNamen );
					return false;
				} else {
					...
					wp_update_post(
						array(
							'ID'        => $new_room_id,
							'post_name' => wp_unique_post_slug( $old_room->meetingName, $new_room_id, 'publish', 'mtz-room', 0 ),
						)
					);
					...
					update_post_meta( $new_room_id, 'mtz-room-moderator-code', $old_room->moderatorPW );
					update_post_meta( $new_room_id, 'mtz-room-viewer-code', $old_room->attendeePW );
					update_post_meta( $new_room_id, 'mtz-room-token', $old_room->meetingID );
					update_post_meta( $new_room_id, 'mtz-room-meeting-id', $meeting_id );
					if ( $old_room_logs_table_exists ) {
						$wpdb->delete( $old_room_logs_table, array( 'meetingID' => $meeting_id ) );
					}

					// Update room recordable value.
					update_post_meta( $new_room_id, 'mtz-room-recordable', ( $old_room->recorded ? 'true' : 'false' ) );
					update_post_meta( $new_room_id, 'mtz-room-wait-for-moderator', ( $old_room->waitForModerator ? 'true' : 'false' ) );
			...
			if ( count( $check_old_rooms ) > 0 ) {
				$this->error_message = __( 'Not all rooms were able to be imported to the new version.', 'meetingz' );
				return false;
			} else {
	...
	private function import_permissions() {
		$old_permissions = get_option( 'meetingz_permissions' );
		if ( false === $old_permissions ) {
			delete_option( 'meetingz_permissions' );
			return;
		}
		foreach ( $old_permissions as $old_role_name => $old_role ) {
			...
			if ( isset( $old_role['manageRecordings'] ) && $old_role['manageRecordings'] ) {
				$role->add_cap( 'manage_mtz_room_recordings' );
				$role->add_cap( 'view_extended_mtz_room_recording_formats' );
			}
			...
			if ( ! $old_role['participate'] ) {
				$role->remove_cap( 'join_as_moderator_mtz_room' );
				$role->remove_cap( 'join_as_viewer_mtz_room' );
				$role->remove_cap( 'join_with_access_code_mtz_room' );
			}
		}
		delete_option( 'meetingz_permissions' );
	}
	...
	private function set_role_participation( $role, $role_name ) {
		switch ( $role_name ) {
			case 'moderator':
				$role->add_cap( 'join_as_moderator_mtz_room' );
				$role->remove_cap( 'join_as_viewer_mtz_room' );
				$role->remove_cap( 'join_with_access_code_mtz_room' );
				break;
			case 'attendee':
				$role->add_cap( 'join_as_viewer_mtz_room' );
				$role->remove_cap( 'join_as_moderator_mtz_room' );
				$role->remove_cap( 'join_with_access_code_mtz_room' );
				break;
			case 'none':
				$role->add_cap( 'join_with_access_code_mtz_room' );
				$role->remove_cap( 'join_as_moderator_mtz_room' );
				$role->remove_cap( 'join_as_viewer_mtz_room' );
				break;
		}
	...
	public function import_from_older_versions() {
		...
		// Set meetingz_plugin_version value.
		update_option( 'meetingz_plugin_version', '1.4.6' );
	...
	public function import_from_older_versions_rooms() {
		...
		$table_name = $wpdb->prefix . 'meetingz';

		// Updates for version 1.3.1 and earlier.
		$meetingz_plugin_version_installed = get_option( 'meetingz_plugin_version' );
		if ( false === $meetingz_plugin_version_installed || ( strcmp( '1.3.1', $meetingz_plugin_version_installed ) <= 0 && get_option( 'mtz_db_version' ) ) ) {

			// Initialize database will create the tables added for version 1.4.6.
			$this->meetingz_init_old_database();
			// Transfer the data from old table to the new one.
			$table_name_old   = $wpdb->prefix . 'mtz_meetingRooms';
			...
			foreach ( $list_of_meetings as $meeting ) {
				...
				$wpdb->query( $wpdb->prepare( $sql, Meetingz_Admin_Helper::generate_random_code( 6 ), $meeting->meetingID, $meeting->meetingVersion, $meeting->attendeePW, $meeting->moderatorPW ) );
			}
			// Remove the old table.
			$wpdb->query( 'DROP TABLE IF EXISTS ' . $table_name_old );

			// Update settings.
			if ( ! get_option( 'mt_mtz_url' ) ) {
				update_option( 'meetingz_url', 'https://api.meetingz.ir/<user>/' );
			} else {
				update_option( 'meetingz_url', get_option( 'mt_mtz_url' ) );
				delete_option( 'mt_mtz_url' );
			}

			if ( ! get_option( 'mt_salt' ) ) {
				update_option( 'meetingz_salt', '8cd8ef52e8e101574e400365b55e11a6' );
			} else {
				update_option( 'meetingz_salt', get_option( 'mt_salt' ) );
				delete_option( 'mt_salt' );
			}

			delete_option( 'mt_waitForModerator' );
			delete_option( 'mtz_db_version' );
		}
	}
	...
	public function import_from_older_versions_permissions() {
		...
		if ( $meetingz_plugin_version_installed && strcmp( $meetingz_plugin_version_installed, '1.3.3' ) < 0 ) {
			$roles              = $wp_roles->role_names;
			$roles['anonymous'] = 'Anonymous';
			$permissions        = [];
			if ( get_option( 'meetingz_permissions' ) ) {
				$old_permissions = get_option( 'meetingz_permissions' );
				...
				update_option( 'meetingz_permissions', $permissions );
			}
		}
	}
	 ...
	 * @since   1.4.6
	 */
	public function meetingz_init_old_database() {
		...
		// Sets the name of the table.
		$table_name      = $wpdb->prefix . 'meetingz';
		$table_logs_name = $wpdb->prefix . 'meetingz_logs';
        ...
        VALUES ('" . Meetingz_Admin_Helper::generate_random_code( 6 ) . "','Demo meeting', '" . time() . "', 'ap', 'mp');";
		dbDelta( $sql );

		$sql = 'INSERT INTO ' . $table_name . " (meetingID, meetingName, meetingVersion, attendeePW, moderatorPW, recorded)
        VALUES ('" . Meetingz_Admin_Helper::generate_random_code( 6 ) . "','Demo meeting (recorded)', '" . time() . "', 'ap', 'mp', TRUE);";
		dbDelta( $sql );
